...
import 'package:flutterui/configs/pkg_colors.dart';
import 'package:flutterui/configs/pkg_sizes.dart';
...
class _PrettyFuzzyButtonState extends State<PrettyFuzzyButton>
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTapUp: (_) async {
  await Future.delayed(Duration(milliseconds: 150)); // <-- Delay for green color to stay
  _controller.reverse();
  setState(() {
    _isHovered = false;
  });
...
onTapCancel: () async {
  await Future.delayed(Duration(milliseconds: 150)); // <-- Same delay here
  _controller.reverse();
  setState(() {
    _isHovered = false;
  });
